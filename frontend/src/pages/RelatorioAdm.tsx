...
export default function RelatorioAdm() {
  ...
  const [summary, setSummary] = useState({ title: "SESSÕES REGISTRADAS", quantidadeSessoes: 0 });
  ...
  useEffect(() => {
    ...
    async function loadReport() {
      try {
        ...
        setSummary({ title: "SESSÕES REGISTRADAS", quantidadeSessoes: dadosMock.geral.totalSessoes });
        ...
        setAtletasCards(dadosMock.atletas.map((item: any) => ({
          title: item.nome, badge: `${item.sessoes} Sessões`, 
  ...
  const renderCategory = (categoryTitle: string, cards: CardData[]) => {
    ...
    return (
      <div style={{ width: '100%', marginTop: '15px' }}>
        ...
        {cards.map((card, idx) => (
          <div key={idx} className={`${styles.sectionCard} ${card.accentClass}`}>
  ...
  if (loading) return <div className={styles.container}><h3>Carregando Relatório...</h3></div>;
    ...
    <div className={styles.container}>
      ...
      <main className={styles.grid}>
        ...
        <div className={styles.chartCard}>
          <span className={styles.chartTitle}>Evolução e Consistência (Taxa de Sudorese)</span>
        ...
        <div className={styles.footerActions} style={{ display: 'flex', gap: '16px', marginTop: '20px' }}>
          <button type="button" className={styles.reportsBtn} style={{ flex: 1, margin: 0 }} onClick={() => navigate(-1)}>Voltar</button>
          <button type="button" className={styles.reportsBtn} style={{ flex: 1, margin: 0 }} onClick={() => navigate("/menu-adm")}>Menu Inicial</button>
        </div>
      </main>
    </div>
  );
